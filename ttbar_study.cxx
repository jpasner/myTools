...
void ttbar_study() {
  ...
  TFile output_file("noHiggsCut_2.0dRcaloJet_ttbar.root","RECREATE"); // Store all output in 1 file
  ...
  // sigFatJet mass histograms
  TH1F *h_sigFatJet_mass = new TH1F("sigFatJet_mass","mass of sigFatJet for all that satisfy triggers and selection",60,0,300);
  TH1F *h_corrected_passCut_sigFatJet_mass = new TH1F("corrected_passCut_sigFatJet_mass","mass of sigFatJet in event that passes hadronic ttbarCR cut after caloJet correction",60,0,300);
  TH1F *h_corrected_noPassCut_sigFatJet_mass = new TH1F("corrected_noPassCut_sigFatJet_mass","mass of sigFatJet in event that fail hadronic ttbarCR cut after caloJet correction",60,0,300);
  TH1F *h_passCut_sigFatJet_mass = new TH1F("passCut_sigFatJet_mass","mass of sigFatJet in event that passes hadronic ttbarCR cut",60,0,300);
  TH1F *h_noPassCut_sigFatJet_mass = new TH1F("noPassCut_sigFatJet_mass","mass of sigFatJet in event that fails hadronic ttbarCR cut",60,0,300);
  ...
  for(int event_itr = 0; event_itr < nEvent; event_itr++) {
    ...
    if((HLT_ht1000_L1J100 || HLT_j420_a10_lcw_L1J100 || HLT_j420_a10r_L1J100 || HLT_j380 || HLT_4j100) &&fatJetPt->size() > 1) {
      if(fatJetPt->at(0) > 400 && fatJetPt->at(1) > 250) {
        ...
        // Fill sigFatJet for all events that pass selection/trigger
        h_sigFatJet_mass->Fill(fatJet4vectors[0].M());
        ...
        for(int vrJet_itr = 0; vrJet_itr < vrJetPt->size(); vrJet_itr++) {
          ...
          if(vrJet4vector.Pt() > 5 && vrJet4vector.DeltaR(fatJet4vectors[1]) < 1.0) {
            // VR track jet pT quality cut and make sure its inside the oppFatJet
        ...
        if(nVRjets == 2) {
          // Fatjets with only 2 VRjets probably have missed some of the top decay
          for(int jet_itr = 0; jet_itr < jetPt->size(); jet_itr++) {
            ...
            if(jet4vector.Pt() > 5 && jet4vector.DeltaR(fatJet4vectors[1]) > 1.0 && jet4vector.DeltaR(fatJet4vectors[1]) < 2.0) {
        ...
        if(nVRjets == 2 && out_jet4vectors.size() > 0) {
          // For oppFatJets missing top decay components we try to add in the closest calo jet (if one exists)
          ...
          corrected_mass = (fatJet4vectors[1] + closest_jet).M();
          h_2vr_corrected_fatJetM->Fill(corrected_mass);
        }

        // oppFatJet Mass plots
        ...
        // sigFatJet Mass plots
        if(corrected_mass > 140 || fatJet4vectors[1].M() > 140) {
          h_corrected_passCut_sigFatJet_mass->Fill(fatJet4vectors[0].M());
        }
        else {
          h_corrected_noPassCut_sigFatJet_mass->Fill(fatJet4vectors[0].M());
        }

        if(fatJet4vectors[1].M() > 140) {
          h_passCut_sigFatJet_mass->Fill(fatJet4vectors[0].M());
        }
        else {
          h_noPassCut_sigFatJet_mass->Fill(fatJet4vectors[0].M());
        }
    ...
    TCanvas *c_sigFatJet_mass = new TCanvas("c_sigFatJet_mass","c_sigFatJet_mass",800,800);
    TLegend *leg_sigFatJet_mass = new TLegend(0.6,0.6,0.9,0.7);
    leg_sigFatJet_mass->AddEntry(h_sigFatJet_mass,"all sigFatJet");
    leg_sigFatJet_mass->AddEntry(h_corrected_passCut_sigFatJet_mass,"passCut with correction");
    leg_sigFatJet_mass->AddEntry(h_corrected_noPassCut_sigFatJet_mass,"failCut with correction");
    leg_sigFatJet_mass->AddEntry(h_passCut_sigFatJet_mass,"passCut");
    leg_sigFatJet_mass->AddEntry(h_noPassCut_sigFatJet_mass,"failCut");
    h_sigFatJet_mass->GetXaxis()->SetTitle("sigFatJetMass [GeV]");
    h_sigFatJet_mass->SetLineColor(1);
    h_corrected_passCut_sigFatJet_mass->SetLineColor(2);
    h_corrected_noPassCut_sigFatJet_mass->SetLineColor(4);
    h_passCut_sigFatJet_mass->SetLineColor(6);
    h_noPassCut_sigFatJet_mass->SetLineColor(8);
    h_sigFatJet_mass->Draw();
    h_noPassCut_sigFatJet_mass->Draw("same");
    h_corrected_passCut_sigFatJet_mass->Draw("same");
    h_corrected_noPassCut_sigFatJet_mass->Draw("same");
    h_passCut_sigFatJet_mass->Draw("same");
    leg_sigFatJet_mass->Draw("same");
    c_sigFatJet_mass->SaveAs("sigFatJet_mass_comparison.pdf");
